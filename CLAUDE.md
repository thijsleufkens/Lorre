...
**Ducking gotcha (mic+system mode)**: enabling voice processing on the mic input activates macOS's *other-audio ducking* by default, which mutes every non-VP audio source (Chrome, Safari, Music, Spotify) system-wide for the duration of the recording — the user hears their meeting/video go silent and the captured system stem is near-empty. This presented as the earlier "Chrome muted / no speech" bug. The fix is `inputNode.voiceProcessingOtherAudioDuckingConfiguration = .init(enableAdvancedDucking: false, duckingLevel: .min)` immediately after `setVoiceProcessingEnabled(true)` (see `startMicrophoneCapture`). VP-aware apps (Teams/Zoom/FaceTime) were never ducked, which is why that case always worked. With ducking minimized, AEC still subtracts the speaker signal from the mic stem; some residual acoustic echo from non-VP sources on built-in speakers may remain (headphones or an external mic eliminate it).
